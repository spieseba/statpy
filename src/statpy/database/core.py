...
class DB:
    def __init__(self, *args, num_proc=None, verbosity=0, sorting_key=lambda x: int(x[0].split("-")[-1]), dev_mode=False):
        ...
        self.database = {} 
        message(subprocess.check_output(['ls'], cwd=os.path.dirname(statistics.__file__)))
    ...
    def add_leaf(self, tag, mean, jks, sample, misc, database=None):
        ...
        if tag not in db or self.debug:
            ...
            # compute mean and jks if necessary
            if mean is None and sample is not None:
                mean = np.mean(self.as_array(sample), axis=0)
            if jks is None and sample is not None:
