...
###################################################################################################################################
    ...
    def jackknife(self, verbose=False):
        ...
        self.covariance = covariance_samples(lambda y: self.estimate_parameters(self.chi_squared, self.estimator(y), self.best_parameter)[0], self.y, self.jk_parameter)
        if verbose:
...
####################################################################################################################################################################
    ...
    def jackknife(self, verbose=False):
        self.jk_parameter = samples(lambda x: self.estimate_parameters(self.t, self.estimator(x), self.W, self.model, self.best_parameter)[0], self.y)
