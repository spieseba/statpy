...
class Fit:
    ...
    def opt_LevenbergMaquardt(self, t, y_est, W, model, p0, verbose=False):
        p, chi2, iterations, success, J = LevenbergMarquardt(t, y_est, W, model, p0, self.min_params)()
        assert success == True
        return p, chi2, J
...
###################################################################################################################################
    ...
    def jackknife(self, parameter_estimator, verbose=False):
        self.jks_parameter = samples(lambda y: parameter_estimator(self.estimator(y))[0], self.y)
    ...
    def fit(self, verbose=True):
        ...
        if self.method in ["Levenberg-Marquardt"]:
            self.best_parameter, self.chi2, self.J = self.opt_LevenbergMaquardt(self.t, self.y_est, self.W, self.model, self.p0, verbose)
            self.best_parameter_cov = self.jackknife(lambda y: self.opt_LevenbergMaquardt(self.t, y, self.W, self.model, self.best_parameter), verbose)
