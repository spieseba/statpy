...
def correlator_exp_fit(t, Ct, cov, p0, symmetric=False, Nt=0, method="Nelder-Mead", minimizer_params={}, shift=0, verbose=True):
    ...
    if method in ["Nelder-Mead", "Migrad"]:
        fitter = sp.fitting.fit(t, Ct, cov, model, p0, lambda x: x, method, minimizer_params=minimizer_params)
    else:
        fitter = sp.fitting.LM_fit(t, Ct, cov, model, p0, lambda x: x, minimizer_params=minimizer_params)
    fitter.fit(verbose)
