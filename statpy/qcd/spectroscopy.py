...
def correlator_double_exp_fit(t, Ct, cov, p0, bc="pbc", Nt=0, method="Nelder-Mead", minimizer_params={}, shift=0, verbose=True):
    ...
    if method in ["Nelder-Mead", "Migrad"]:
        fitter = sp.fitting.fit(t, Ct, cov, model, p0, lambda x: x, method=method, minimizer_params=minimizer_params)
        best_parameter, chi2 = fitter.estimate_parameters(fitter.chi_squared, Ct, fitter.p0)
    else:
        fitter = sp.fitting.LM_fit(t, Ct, cov, model, p0, lambda x: x, minimizer_params=minimizer_params)
        best_parameter, chi2, _ = fitter.estimate_parameters(fitter.t, Ct, fitter.W, fitter.model, fitter.p0, verbose)
...
def const_fit(t, y, cov, p0, method="Nelder-Mead", minimizer_params={}, error=True, verbose=True):
    ...
    if method in ["Nelder-Mead", "Migrad"]:
        ...
        fitter = sp.fitting.LM_fit(t, y, cov, model, p0, estimator=lambda x: x, minimizer_params=minimizer_params)
        fitter.fit(verbose, error)
    if error:
