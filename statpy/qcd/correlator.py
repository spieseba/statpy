...
#def lc_combined_cosh_sinh_fit(db, tag0, tag1, B, t0, t1, p0, correlated=False, fit_method="Migrad", fit_params=None, jks_fit_method=None, jks_fit_params=None, verbosity=0, make_plot=True):
#    A_PS_dict = {}; A_PS_var_dict = {}; A_PS_jks_dict = {}
#    A_A4_dict = {}; A_A4_var_dict = {}; A_A4_jks_dict = {}
#    m_dict = {}; m_var_dict = {}; m_jks_dict = {}
#    # estimate cov using unbinned sample
#    jks0_ub = db.compute_sample_jks(tag0, 1); jks1_ub = db.compute_sample_jks(tag1, 1)
...
#        print(f"BINSIZE = {b}\n")
#        jks0 = db.compute_sample_jks(tag0, binsize=b); jks1 = db.compute_sample_jks(tag1, binsize=b)
...
#            print("-----------------------------------------------------------------------------------------\n")
#        if b == B:
#            lc_combined_plot(db, tag_PSPS, Ct_mean_PSPS, Ct_jks_PSPS, fit_range_PSPS, model_combined, best_parameter, best_parameter_jks, best_parameter_cov)
#
...
def local_amp(trange, model, model_type, best_parameter, best_parameter_jks, Nt):
    ...
    At = effective_amp(Ct_fit, trange, best_parameter[1], Nt) 
    At_jks = {}
    for j in best_parameter_jks:
        Ct_fit_jks = np.array([model(t, best_parameter_jks[j]) for t in trange])
        At_jks[j] = effective_amp(Ct_fit_jks, trange, best_parameter_jks[j][1], Nt)  
...
def lc_plot(db, Ct_tag, Ct_mean, Ct_jks, binsize, fit_range, model_type, model, best_parameter, best_parameter_jks, best_parameter_cov):
    ...
    # data
    color = "C2"
    At_func = lambda x, y: effective_amp_cosh(x[d:Nt-d], np.arange(d, Nt-d), y, Nt)
...
def lc_combined_plot(db, tag_PSPS, Ct_mean_PSPS, Ct_jks_PSPS, fit_range_PSPS, model_combined, best_parameter, best_parameter_jks, best_parameter_cov):
