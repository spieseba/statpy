...
class LevenbergMarquardt:
    ...
    """
    def __init__(self, data, model, p0, lm_parameter_user={}):
        self.t, self.y, self.stds = data
        self.model = model
        self.p0 = p0
        ...
        self.C = lm_parameter["C"]
        # Weight matrix = inverse of the covariance matrix
        if self.C is not None:
            self.W = np.linalg.inv(self.C)
        else:
            self.W = np.diag(1 / self.stds**2)
    ...
    def chi_squared(self, p):
        return np.sum( ((self.model(self.t, p) - self.y) / self.stds )**2 )
